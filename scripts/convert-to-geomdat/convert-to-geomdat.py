...
#                           print line[c:c+4]
    ...
    with open("Geometry-" + str(i) + ".dat", "w" ) as f:
        ...
        for j in range(0,atomnum):
            if namelist[j] == "Cl-":
                f.write( '{0:<10s}'.format(namelist[j][0:2]) + ''.join( '{0:>22.16f}'.format(x/0.52917724924) for x in  atomcoord[ j ] ) + '\n' )
            else:
               f.write( '{0:<10s}'.format(namelist[j][0]) + ''.join( '{0:>22.16f}'.format(x/0.52917724924) for x in  atomcoord[ j ] ) + '\n' )
        f.write( "# momenta\n")
        for j in range(0,atomnum):
           for x in atomvel[ j ]:
              p = x * atommass[j]  * 9.3499611711905144E-04  * 1.8228884855409500E+03 
              f.write(  '{0:>22.16f}'.format( p ) )  # time unit in rst7: 1/20.455 ps   length unit in rst7: angstrom    mass unit in amber: atomic mass unit
           f.write('\n')
